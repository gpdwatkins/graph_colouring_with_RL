...
#                 # no_orig_vertices=None, no_col_vertices=None,
#                 **kwargs):
...
#         # self.no_orig_vertices = no_orig_vertices
#         # self.no_col_vertices = no_col_vertices

#     def __inc__(self, key, value, *args, **kwargs):
#         # if key == 'orig_orig_edge_index':
#         #     return torch.tensor([[self.no_orig_vertices], [self.no_orig_vertices]])
#         # elif key == 'orig_col_edge_index':
#         #     return torch.tensor([[self.no_orig_vertices], [self.no_col_vertices]])
#         # elif key == 'col_orig_edge_index':
#         #     return torch.tensor([[self.no_col_vertices], [self.no_orig_vertices]])
#         # else:
...
# # def FlattenEdges(edges_df):
# #     # Converts a dataframe to a vector
# #     # Note that the edges in this environment are undirected so only the elements above the diagonal are included in the list of edge values
# #     row_names = edges_df.index.values
# #     col_names = edges_df.columns.values
# #     output = []
# #     for row_ind, row_name in enumerate(row_names):
# #         for col_ind, col_name in enumerate(col_names[row_ind+1:]):
# #             # if row_ind != col_ind:
# #             if type(edges_df.at[row_name, col_name]) is list:
# #                 output += edges_df.at[row_name, col_name]
# #             else:
# #                 output += [edges_df.at[row_name, col_name]]
...
# # def GenerateStateFromGraph(current_graph, target_edges):
# #     vertex_names = list(current_graph.edges.index)
    
# #     target_edges_flat = FlattenEdges(target_edges)
# #     current_edges_flat = FlattenEdges(current_graph.edges)
# #     vertex_values_list = [current_graph.vertices[vertex_name] for vertex_name in vertex_names]

# #     current_state = target_edges_flat + current_edges_flat + vertex_values_list
...
#     # print('target_edges.index.values:')
#     # print(target_edges.index.values)
#     # print('target_edges.columns.values:')
#     # print(target_edges.columns.values)
#     # print('state_edges.index.values:')
#     # print(state_edges.index.values)
#     # print('state_edges.columns.values:')
#     # print(state_edges.columns.values)
...
# # def all_vertices_assigned(vertices, vertex_names):
# #     # for vertex_name, colour_name in vertices.items():
# #     #     if colour_name=='-1':
# #     #         return 0
# #     # return 1
...
# #         if vertices[vertex_name] == '-1':
# #             return 0
    
# #     return 1


# # def no_matching_edges(edges1, edges2):
# #     no_matching_elts = 0
# #     for row_label in edges1.index.values:
# #         for col_label in edges1.columns.values:
# #             elt1 = edges1.at[row_label, col_label]
# #             elt2 = edges2.at[row_label, col_label]
# #             if not (isnan(elt1) or isnan(elt2)):
# #                 if elt1==elt2:
# #                     no_matching_elts += 1
# #     # Divide the number of matching elts by 2 because edge [i,j] has same value as edge [j,i] so we'd be double counting
# #     return no_matching_elts/2


# # def graph_score(edges1, edges2):
# #     score = 0
# #     for row_ind, row_label in enumerate(edges1.index.values):
# #         for col_label in edges1.columns.values[row_ind+1:]:
# #             elt1 = edges1.at[row_label, col_label]
# #             elt2 = edges2.at[row_label, col_label]
# #             # Edge score of 0 means one of the vertices is unassigned
# #             if not (elt1 == 0 or elt2 == 0):
# #                 if elt1 == elt2:
# #                     score+=1
# #                 else:
# #                     score-=1
...
# def remaining_seats_on_table(vertex_values, table_size, table_no):
#     remaining_seats = table_size
#     for table in vertex_values.values():
#         if table == table_no:
#             remaining_seats-=1
...
# def no_guests_per_table(no_tables, vertex_values):
#     output_dict = {}
#     for table_no in range(no_tables):
#         output_dict[str(table_no)] = 0
#     for table_no in vertex_values.values():
#         if not table_no == -1:
#             output_dict[str(table_no)] += 1
#     return output_dict


# def all_tables_full(no_tables, table_size, vertex_values):
#     guests_per_table = no_guests_per_table(no_tables, vertex_values)
#     tables_full = True
#     for guests_on_table in guests_per_table.values():
#         if not guests_on_table == table_size:
#             tables_full = False
#             break
#     return tables_full


# # def convert_colour_to_onehot(colour, no_colours, no_orig_vertices):
# #     output = [0 for _ in range(no_colours)]
# #     colour_ind = int(colour)
# #     if not colour_ind == -1:
# #         output[colour_ind-no_orig_vertices] = 1
# #     return output

# # def convert_vertex_name_to_onehot(vertex_ind, no_orig_vertices):
# #     output = [0 for _ in range(no_orig_vertices)]
# #     output[vertex_ind] = 1
# #     return output
...
# # def import_dimacs_graph(full_graph_filepath, possible_vertex_labels):
...
#     # vertex_labels = random.sample(possible_vertex_labels, no_orig_vertices)

#     # vertex_degrees = get_vertex_degrees(no_orig_vertices, target_edge_list, target_edge_indices)

#     # target = {
#     #     # 'vertex_labels': vertex_labels,
#     #     'vertex_degrees': vertex_degrees,
#     #     'edge_list': tuple(target_edge_list),
#     #     'edge_attr': tuple(target_edge_attr),
#     #     'target_edge_indices': tuple(set(target_edge_indices))
#     #     }

#     # return no_orig_vertices, target, edge_ind_dict
...
# # def generate_graph_image(vertex_values, networkx_graph, colours):
# #     available_colour_codes = ['#e6194B', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#42d4f4', '#f032e6', '#fabed4', '#469990', '#dcbeff', '#9A6324', '#fffac8', '#800000', '#aaffc3', '#000075', '#a9a9a9', '#000000']
# #     colour_dict = {colour:colour_code for colour, colour_code in zip(colours, available_colour_codes[:len(colours)])}
# #     colour_dict['-1'] = '#ffffff'

# #     vertex_colours = [colour_dict[vertex_values[node]] for node in networkx_graph.nodes]

# #     fig1 = plt.figure(figsize=(10,10))
# #     nodes.set_edgecolor('r')
# #     # nx.draw_circular(networkx_graph, with_labels=True, node_color=vertex_colours)
# #     nx.draw_circular(networkx_graph, node_color=vertex_colours)
# #     return fig1
...
#             # target_vertex_values = self.GenerateTargetVertices(validation_graph_vertex_names, validation_graph_colour_names)
...
#             # validation_graph_vertex_names = [i for i in range(validation_graph_no_vertices)]
#             # validation_graph_colour_names = [str(i) for i in range(validation_graph_no_colours)]
...
#     # print('degree_assortativity_coefficient: ', degree_assortativity_coefficient)
#     # print('transitivity: ', transitivity)
#     # print('diameter: ', diameter)
#     # print('radius: ', radius)
#     # print('average_degree: ', average_degrees['lin'])
#     # print('neighbour_distance_counts_scaled_avg: ', neighbour_distance_counts_scaled_avg)
#     # print('clustering_coefficients_avg: ', clustering_coefficients_avg)
#     # print('average_neighbor_degree_avg: ', average_neighbor_degree_avg)
#     # print('closeness_centrality_avg: ', closeness_centrality_avg)
#     # print('betweenness_centrality_avg: ', betweenness_centrality_avg)
#     # print('current_flow_closeness_centrality_avg: ', current_flow_closeness_centrality_avg)
#     # print('current_flow_betweenness_centrality_avg: ', current_flow_betweenness_centrality_avg)
#     # print('eigenvector_centrality_avg: ', eigenvector_centrality_avg)
#     # print('katz_centrality_avg: ', katz_centrality_avg)
#     # print('load_centrality_avg: ', load_centrality_avg)
#     # print('triangles_avg: ', triangles_avg)
#     # print('square_clustering_avg: ', square_clustering_avg)
#     # print('core_number_avg: ', core_number_avg)
#     # print('eccentricity_avg: ', eccentricity_avg)
#     # print('communicability_betweenness_centrality_avg: ', communicability_betweenness_centrality_avg)
#     # input('press enter')

#     # edge features 
#     # (should be list of length n(n-1) in same order as edge_list)
