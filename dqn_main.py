...
from utils import save_benchmark_stats_to_file, save_stats_to_file, save_validation_stats_to_file
...
for manual_seed, exp_name, training_run_readme in zip(manual_seeds, exp_names, training_run_readmes):
    ...
    if SAVE_OUTPUTS:
        ...
        print('Saving output to directory', base_dir)
        os.mkdir(base_dir)
    ...
    _, example_target, example_current_graph = env.reset()
    ...
    for ep_ind in range(1, agent.no_episodes+1):
        ...
        _, target, current_graph = env.reset()
        ...
        if SAVE_OUTPUTS:
            ...
            if ep_ind>0 and ((ep_ind%1000 == 0) or (ep_ind == agent.no_episodes)):
                ...
                save_stats_to_file(training_stats, training_stats_filename)
